...
bool BaseHTTPProtocol::HandleChunkedContent(IOBuffer &buffer) {
	...
	while (GETAVAILABLEBYTESCOUNT(buffer) >= 3) {
		...
		if (chunkSize > HTTP_MAX_CHUNK_SIZE) {
			FATAL("Chunk size too large. Maximum allowed is %""I32u"" and we got %""I32u",
		...
		DEBUG_HTTP("available bytes before ignore: %""I32u", GETAVAILABLEBYTESCOUNT(buffer));
		...
		DEBUG_HTTP("available bytes  after ignore: %""I32u", GETAVAILABLEBYTESCOUNT(buffer));
