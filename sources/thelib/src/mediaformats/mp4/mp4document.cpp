...
BaseAtom * MP4Document::ReadAtom(BaseAtom *pParentAtom) {
	...
	if (currentPos + pAtom->GetSize() != _mediaFile.Cursor()) {
		if (currentPos + pAtom->GetSize() < _mediaFile.Cursor()) {
			FATAL("atom overflow: atom start: %""I64u""; Atom size: %""I64u""; currentPos: %""I64u",
					currentPos, pAtom->GetSize(), _mediaFile.Cursor());
			return NULL;
		} else {
			WARN("wasted space inside atom! atom start: %""I64u""; Atom size: %""I64u""; currentPos: %""I64u",
					currentPos, pAtom->GetSize(), _mediaFile.Cursor());
...
bool MP4Document::BuildFrames() {
	...
	if (pESDS != NULL) {
		...
		//		FINEST("Start: %""I64u"" (%"PRIx64"); Length: %""I64u"" (%"PRIx64");",
...
bool MP4Document::BuildMOOVFrames(bool audio) {
	...
	if (pCTSS != NULL) {
		...
		if (sampleSize.size() != compositionOffsets.size()) {
			WARN("composition offsets count != samples count; compositionOffsets: %""I""u; sampleSize.size: %""I""u",
			...
			WARN("composition offsets padded with 0. Now size is %""I""u",
					compositionOffsets.size());
		}
	}
	INFO("audio: %d; keyFrames: %""I""u; frames: %""I""u; compositionOffsets: %""I""u",
