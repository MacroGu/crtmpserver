...
string Variant::ToString(string name, uint32_t indent) {
	...
	case V_INT64:
	{
		result += format("%s<INT64 name=\"%s\">%""I64d""</INT64>",
	...
	case V_UINT64:
	{
		result += format("%s<UINT64 name=\"%s\">%""I64u""</UINT64>",
	...
	case V_BYTEARRAY:
	{
		result += format("%s<BYTEARRAY name=\"%s\">%""I""u bytes</BYTEARRAY>",
...
Variant::operator string() {
	...
	case V_INT64:
	{
		return format("%""I64d", this->operator int64_t());
	}
	case V_UINT8:
	case V_UINT16:
	case V_UINT32:
	...
	case V_UINT64:
	{
		return format("%""I64u", this->operator uint64_t());
	}
	case V_DOUBLE:
	{
...
Variant& Variant::operator[](Variant &key) {
	...
	{
		char temp[11];
		sprintf(temp, "0x%08""I32x", (uint32_t)key);
		return operator[](temp);
	}
	case V_STRING:
	{
...
uint32_t Variant::MapDenseSize() {
	...
	for (denseCount = 0; denseCount < MapSize(); denseCount++) {
		char temp[11];
		sprintf(temp, "0x%08""I32x", denseCount);
...
TiXmlElement *Variant::SerializeToXmlElement(string &name) {
	...
	case V_INT64:
	{
		...
		pResult->LinkEndChild(new TiXmlText(format("%""I64d", _value.i64)));
		break;
	}
	case V_UINT8:
	{
	...
	case V_UINT64:
	{
		...
		pResult->LinkEndChild(new TiXmlText(format("%""I64u", _value.ui64)));
		break;
	}
	case V_DOUBLE:
	{
...
bool Variant::DeserializeFromXml(TiXmlElement *pNode, Variant &variant) {
	...
	if (nodeName == "bool") {
		...
		if (sscanf(STR(text), "%""I64d", &val.i64) != 1) {
		...
		if (sscanf(STR(text), "%""I64u", &val.ui64) != 1) {
			FATAL("Invalid number");
			return false;
		}
