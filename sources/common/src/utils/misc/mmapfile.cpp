...
bool MmapPointer::Allocate(int fd, uint64_t cursor,
	...
	if (size > windowSize) {
		FATAL("size is greater than window size: %""I64u"" > %u", size, windowSize);
		return false;
	}
	...
	while (_cursor + _size < cursor + size) {
		LOG_MMAP("We need to go a little bit further; Wanted: %""I64u""; got: %""I64u",
				cursor + size, _cursor + _size);
		_size += MmapFile::_pageSize;
	}

	LOG_MMAP("Reading %""I64u"" bytes from disk", _size);
...
MmapPointer::operator string() {
	if (_size == 0)
		return "[N - N](0)";
	return format("[%""I64u"" - %""I64u""](%u)", _cursor, _cursor + _size - 1, _size);
}
...
bool MmapFile::SeekTo(uint64_t position) {
	...
	if (position > _size) {
		FATAL("Invalid position: %""I64u"". Must be at most: %""I64u", position, _size - 1);
		_failed = true;
		return false;
	}
	_cursor = position;
...
bool MmapFile::PeekBuffer(uint8_t *pDestBuffer, uint64_t count) {
	//1. Sanity checks
	if (_failed) {
		DEBUG("_cursor: %""I64u""; count: %""I64u""; %s", _cursor, count, STR(_path));
		FATAL("This mmap file is in inconsistent state");
		return false;
	}
	...
	if (_windowSize < count) {
		DEBUG("_cursor: %""I64u""; count: %""I64u""; %s", _cursor, count, STR(_path));
		FATAL("Invalid window size: _windowSize < count %u < %""I64u",
				_windowSize, count);
		_failed = true;
		return false;
	}
	...
	if (_cursor + count > _size) {
		DEBUG("_cursor: %""I64u""; count: %""I64u""; %s", _cursor, count, STR(_path));
		FATAL("EOF will be reached: cursor: %""I64u""; count: %""I64u""; size: %""I64u",
				_cursor, count, _size);
		_failed = true;
		return false;
	}
	...
	if (pPointer->Copy(pDestBuffer, _cursor, 0, count) != count) {
		FATAL("Unable to copy %""I64u"" bytes", count);
		_failed = true;
		return false;
	}
